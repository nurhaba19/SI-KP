...
class Admin extends BaseController{
    ...
    public function tambahpembimbing_action(){
        ...
        if(count($this->ModelsAdmin->CheckUsernamePembimbing($_POST['username'])->getResult()) == 0){

            if(count($_POST['id_industri']) == 0){

                echo json_encode('Error; industri belum dipilih');
                ...
                foreach(@$_POST['id_industri'] as $_id_industri){
    
                    $this->ModelsAdmin->penempatanpembimbing(
                        $this->ModelsAdmin->getidpembimbingByNamaNipUsername($_POST['nama_pembimbing'], $_POST['nip'], $_POST['username']),
                        $_id_industri
                    );
            ...
            echo json_encode('Maaf username sudah digunakan');
    ...
    public function editpembimbing_action(){
        ...
        if(!empty($_POST['id_industri'])){

            $this->ModelsAdmin->bersihkanPenempatanPembimbing($_POST['id']);
        ...
        if(isset($_FILES['excel']['name']) && in_array($_FILES['excel']['type'], $file_mimes)) {
            ...
            try{
                ...
                for($i = 1;$i < count($sheetData);$i++){
                    ...
                    if(count($this->ModelsAdmin->CheckUsernamePembimbing($sheetData[$i]['5'])->getResult()) == 0){
    
                        $_industri = explode('|', $sheetData[$i]['3']);
                        ...
                        for ($j=0; $j<count($_industri); $j++){
        
                            $this->ModelsAdmin->penempatanpembimbing(
                                $this->ModelsAdmin->getidpembimbingByNamaNipUsername($sheetData[$i]['2'], $sheetData[$i]['1'], $sheetData[$i]['5']),
                                $_industri[$j]
                            );
    ...
    public function updatejurnalharian(){
        ...
        if(empty($_POST['id_jurnal_harian'])){
            ...
            if(empty($this->input->getPost('catatan'))){
